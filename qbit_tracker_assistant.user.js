...
// @version       2.1
// @description   集成功能：1.失效种子自动打标；2.Tracker地址批量替换；3.按分类批量转移存储位置（安全确认版）。
// @author        DRH
// --- 在下方修改或添加您的 qBittorrent 访问地址 ---
// @match         http://192.168.124.50:18765/
// @grant         none
// @run-at        document-end
...
(function() {
    ...
    const injectUI = () => {
        ...
        panel.innerHTML = `
            ...
            <b style="color:#00bcd4;font-size:16px;">qBittorrent 助手 v2.1</b><br>
            ...
            <div id="transfer-panel" style="display:none; margin-top:10px;">
                ...
                <button id="import-cat-btn" style="width:100%;cursor:pointer;background:#ffc107;color:black;border:none;padding:10px;font-weight:bold;border-radius:4px;margin-bottom:12px;">📂 导入全部分类</button>
        ...
        try {
            ...
            catNames.forEach(name => {
                ...
                row.innerHTML = `
                    ...
                    <input type="text" class="cat-path-input" value="${path}" style="flex:2; padding:4px; background:#333; border:1px solid #555; color:#fff; border-radius:3px; font-size:11px;">
                `;
    ...
    async function runBrokenSeedScan() {
        ...
        const usePeers = document.getElementById('check-peers').checked, useStatus = document.getElementById('check-status').checked;
        if (!usePeers && !useStatus) { alert("⚠️ 请勾选条件！"); return; }
            ...
            for (let i = 0; i < count; i += chunk) {
                ...
                const res = await Promise.all(torrents.slice(i, i + chunk).map(async t => {
                    try {
                        ...
                        if (real.length > 0 && real.every(tr => (usePeers && tr.num_peers === -1) || (useStatus && (tr.status === 1 || tr.msg.includes("未工作"))))) return t.hash;
                    } catch(e) {} return null;
                }));
